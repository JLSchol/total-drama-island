...
def kalman_filter_smoothing(df: pd.DataFrame, column_name: str, product_col: str = 'product', filter: str = "2D", smoothing_type="EMA", window=2) -> pd.DataFrame:
    ...
    for product in df[product_col].unique():
        ...
        if smoothing_type == "SMA":
            df[smoothed_kalman_col] = df.groupby(product_col)[kalman_col].transform(lambda x: x.rolling(window=window).mean())
...
def add_exponential_moving_average(df, column_name, window, alpha=None):
    ...
    - column_name: The name of the column to apply EMA to.
    - window: The window size for the EMA.
    - alpha: The smoothing factor (between 0 and 1). If None, it will be computed from the window size.

    Returns:
    - df: The DataFrame with the EMA column added.
    - column_name_ema: The name of the added EMA column.
    """
    column_name_ema = f"{column_name}_ema_{window}"
...
def add_momentum_signal(df, column_name, window):
    """
    Adds a momentum signal based on the price difference (delta) to detect momentum shifts.
    - 1 indicates a change from negative to positive delta (momentum shift from down to up).
    - -1 indicates a change from positive to negative delta (momentum shift from up to down).
    - 0 indicates no momentum shift.
    
    The momentum signal is shifted forward by one period to predict the future momentum for each product.

    Args:
    - df: The DataFrame containing the price data.
    - column_name: The column that holds the price data.
    - window: The size of the rolling window to calculate local highs and lows.
    - product_column: The name of the column that indicates product groups.

    Returns:
    - df: DataFrame with an added column 'momentum_signal' for momentum direction.
    """
    # Calculate the difference (delta) between consecutive price values
    df['price_diff'] = df.groupby("product")[column_name].diff()

    # Initialize the momentum_signal column with zeros
    df['momentum_signal'] = 0

    # Calculate momentum signal based on the price differences, grouped by product
    for product, group in df.groupby("product"):
        for i in range(1, len(group) - 1):  # Skip first and last because they lack neighbors
            # Check if the delta changes from negative to positive (momentum shift from down to up)
            if group['price_diff'].iloc[i-1] < 0 and group['price_diff'].iloc[i] > 0:
                df.loc[group.index[i], 'momentum_signal'] = 1  # Buy signal (momentum shift up)
            
            # Check if the delta changes from positive to negative (momentum shift from up to down)
            elif group['price_diff'].iloc[i-1] > 0 and group['price_diff'].iloc[i] < 0:
                df.loc[group.index[i], 'momentum_signal'] = -1  # Sell signal (momentum shift down)

    # Shift the momentum signal forward by one period to predict future momentum
    df['momentum_signal'] = df.groupby("product")['momentum_signal'].shift(-1)

    # Optionally drop 'price_diff' if you don't need it
...
if __name__ == "__main__":
    ...
    df, km3, km3s =  kalman_filter_smoothing(df, column_name="mid_price", product_col="product", filter="2D", smoothing_type="SMA", window=10)
