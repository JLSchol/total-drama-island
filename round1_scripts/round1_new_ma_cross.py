...
def get_best_order(order_type: str, product: str, price: int, amount: int, current_position: int, max_position: int, orders: List[Order]) -> List[Order]:
    ...
    if not is_available(price, amount):
        ...
        return orders, None, None  # No valid price or quantity available
    ...
    if order_type == "buy":
        ...
        if order_quantity <= 0:
            ...
            return orders, remaining_capacity, order  # No valid buy order
    else:  # Sell order
        ...
        if order_quantity >= 0:
            ...
            return orders, remaining_capacity, order  # No valid sell order

    # Append the order
    order = Order(product, int(price), int(order_quantity))
    orders.append(order)

    return orders, remaining_capacity, order
...
def get_best_orders(product: str, fair_price: float, 
    ...
    if best_ask is not None and best_ask < fair_price:
        orders, _, _ = get_best_order("buy", product, best_ask, best_ask_volume, current_position, max_buy_position, orders)
    # Place sell order if best bid is higher than the fair price
    if best_bid is not None and best_bid > fair_price:
        orders, _, _ = get_best_order("sell", product, best_bid, best_bid_volume, current_position, max_sell_position, orders)
    return orders
...
def momentum(prices: np.ndarray, period: int) -> float:
    # Check for valid input
    if prices.size == 0 or period <= 0:
        raise ValueError("Prices array is empty or period size is invalid")
    
    if prices.size == 1:
        return 0.0

    # Adjust period size if it's larger than available prices
    period = min(period, prices.size)
    
    # Calculate the momentum, ignoring NaNs
    momentum = (prices[-1] - prices[-period]) / prices[-period]  # This will ignore NaN values when calculating the mean
...
# def add_order_data_to_trading_data(td: TradingData, product: str, order: Order):
#     quantity = order.quantity
#     price = order.price
#     revenu = price * quantity
    
#     td.apply_indicator(product, "order_quantity", quantity)
#     td.apply_indicator(product, "order_price", price)
#     td.apply_indicator(product, "order_revenu", revenu)

def add_orders_to_trading_data(td: TradingData, product: str, orders: List[Order]):
    if not orders:
        return
    quantities = []
    prices = []
    ...
    for order in orders:
        if order.symbol == product:
            ...
            revenues.append(order.price * order.quantity)

    # ✅ Apply indicators to this specific product only (only set best orders)
    td.apply_indicator(product, "order_quantity", quantities[0])
    td.apply_indicator(product, "order_price", prices[0])
    td.apply_indicator(product, "order_revenue", revenues[0])
    # td.apply_indicator(product, "total_revenues", sum(revenues))

def reset_order_data_in_trading_data(td: TradingData, product: str):
    td.apply_indicator(product, "order_quantity", 0)
    td.apply_indicator(product, "order_price", 0)
    td.apply_indicator(product, "order_revenue", 0)

def sma_cross_momentum_strategy(td: TradingData, product: str, orders: list[Order],
                            sma_fast_l, sma_slow_l, mom_period, mom_threshold,
    ...
    # define names
    sma_fast_name = f"sma_{sma_fast_l}"
    sma_slow_name = f"sma_{sma_slow_l}"
    sma_cross_signal_name = f"sma_cross{sma_fast_l}_{sma_slow_l}_signal"
    sma_cross_mom_signal_name = f"sma_cross{sma_fast_l}_{sma_slow_l}_mom{mom_period}_{mom_threshold}_signal"

    # get previous sma step, momentum
    previous_sma_fast = td.get_last_field(product, sma_fast_name)
    previous_sma_slow = td.get_last_field(product, sma_slow_name)
    previous_sma_cross_signal = td.get_last_field(product, sma_cross_signal_name)
    previous_sma_cross_mom_signal = td.get_last_field(product, sma_cross_mom_signal_name)

    # if they did not exist yet, create them and initialize them with the latest price
    if previous_sma_fast is None and previous_sma_slow is None:
        td.apply_indicator(product, sma_fast_name, prices[-1])
        td.apply_indicator(product, sma_slow_name, prices[-1])
        td.apply_indicator(product, sma_cross_signal_name, 0)
        td.apply_indicator(product, sma_cross_mom_signal_name, 0)
        return orders
    
    # calculate current step
    sma_fast = sma(prices, sma_fast_l)
    sma_slow = sma(prices, sma_slow_l)
    # calculate the momentum (over the last mom_period)
    mom = momentum(prices, mom_period)

    # signal if fast above slow (1), or below
    if sma_fast > sma_slow:
        sma_cross_signal = 1
    elif sma_fast < sma_slow:
        sma_cross_signal = -1
    elif sma_fast == sma_slow:
        sma_cross_signal = previous_sma_cross_signal

    # more soffisitcated signal: 
    # if fast above slow and momentum is positive (1)
    if sma_cross_signal == 1 and mom > mom_threshold:
        sma_cross_mom_signal = 1
    elif sma_cross_signal == -1 and mom < -mom_threshold:
        sma_cross_mom_signal = -1
    else:
        # hold current signal
        sma_cross_mom_signal = previous_sma_cross_mom_signal

    # apply indicators
    td.apply_indicator(product, sma_fast_name, sma_fast)
    td.apply_indicator(product, sma_slow_name, sma_slow)
    td.apply_indicator(product, sma_cross_signal_name, sma_cross_signal)
    td.apply_indicator(product, sma_cross_mom_signal_name, sma_cross_mom_signal)
    
    # if position limit not reached, buy best bid or ask depending on the signal
    current_position = td.get_last_field(product, "current_position")
    max_buy_position = td.get_last_field(product, "max_buy_position")
    max_sell_position = td.get_last_field(product, "max_sell_position")

    # keep buying best bids for current position
    # TODO: test and add fair price (to prevent buying in a peak)
    # TODO: add buy everything ()
    # TODO: test to see if need finish on 0 position
    if sma_cross_mom_signal == 1 and int(current_position) < int(max_buy_position) :
        best_ask = td.get_last_field(product, "best_ask")
        best_ask_volume = td.get_last_field(product, "best_ask_volume")
        if best_ask is not None and best_ask < fair_price:
            orders, _, order= get_best_order("buy", product, best_ask, best_ask_volume, current_position, max_buy_position, orders)
            # add_order_data_to_trading_data(td, product, order)

    if sma_cross_mom_signal == -1 and int(current_position) > int(max_sell_position):
        best_bid = td.get_last_field(product, "best_bid")
        best_bid_volume = td.get_last_field(product, "best_bid_volume")
        if best_bid is not None and best_bid > fair_price:
            orders, _, order = get_best_order("sell", product, best_bid, best_bid_volume, current_position, max_sell_position, orders)
            # add_order_data_to_trading_data(td, product, order)
...
class Trader:
    def run(self, state: TradingState):
        ...
        # Initialize per-product order tracking to zero

        for product in state.order_depths:
            orders: List[Order] = []
            reset_order_data_in_trading_data(td, product)
            ...
            if product == "SQUID_INK":
                # pass
                sma_fast, sma_slow, mom_period, mom_threshold = 16, 27, 28, 0.0045
                orders = sma_cross_momentum_strategy(td, product, orders, 
                                                 sma_fast, sma_slow, mom_period, mom_threshold,
                                                 "mid_price")
                
            add_orders_to_trading_data(td, product, orders)
